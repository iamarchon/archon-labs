...
const AnimatedPrice = ({ value }) => {
  ...
  return (
    <span style={{ transition:"color .45s ease", color: flash==="up" ? T.green : flash==="down" ? T.red : T.ink, fontVariantNumeric:"tabular-nums" }}>
      ${value.toFixed(2)}
    </span>
  );
};
  ...
  return (
    <header style={{
      ...
      <div style={{ maxWidth:"1020px", margin:"0 auto", height:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 28px" }}>
        <div onClick={() => setPage("dashboard")} style={{ fontSize:"17px", fontWeight:700, letterSpacing:"-0.4px", color:T.ink, cursor:"pointer", userSelect:"none" }}>
...
/* ─────────────────────────────────────────────────────────────────────────────
   MARKETS  —  now powered by StockSearch
───────────────────────────────────────────────────────────────────────────── */
const MarketsPage = ({ onTrade, watchlist, onWatch }) => (
    ...
    <div style={{ maxWidth:"860px", margin:"0 auto", padding:"40px 28px 100px" }}>
      ...
      <Reveal delay={0.14}>
        ...
        <Card hover={false} style={{ overflow:"hidden" }}>
          {HOLDINGS.map((h,i) => {
            const s=stocks.find(x=>x.ticker===h.ticker); if(!s) return null;
            const val=s.price*h.shares, cost=h.avgCost*h.shares, g=val-cost, gPct=(g/cost)*100;
            return (
...
export default function SwishApp() {
  ...
  const PAGES = {
    ...
    markets:     <MarketsPage     onTrade={setTradeStock} watchlist={watchlist} onWatch={handleWatch} />,
