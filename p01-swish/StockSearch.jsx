...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Full-market stock search powered by Finnhub symbol search API.
   Shows curated featured stocks by default.
   Debounced live search on any query — name or ticker.
   Apple-grade UI: clean white dropdown, hairline shadows, smooth transitions.
   
   Props:
     onTrade(stock)   — called when user clicks Trade on a result
     onWatch(stock)   — called when user clicks Watch on a result
     apiKey           — Finnhub API key (from import.meta.env.VITE_FINNHUB_API_KEY)
─────────────────────────────────────────────────────────────────────────────── */
...
/* ── Curated featured stocks shown before any search ── */
const FEATURED = [
...
export default function StockSearch({ onTrade, onWatch, watchlist = [] }) {
  ...
  const [searchMode, setSearchMode] = useState(false); // true = live API search
  const debounceRef                 = useRef(null);
  const inputRef                    = useRef(null);

  /* ── Filtered featured list (no search query) ── */
  ...
  const doSearch = useCallback(async (q) => {
    ...
    try {
      ...
      if (url) {
        ...
        /* Fallback: filter FEATURED by query when no API key yet */
      ...
      /* On error fall back to featured filter */
      const q2 = q.toLowerCase();
  ...
  return (
    <div>
      ...
      {/* ── Sector pills (only when not in search mode) ── */}
      {!searchMode && (
